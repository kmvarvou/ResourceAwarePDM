...
public class KnockoutPathPruneCombo extends KnockoutPathPruneHeuristic {
    public KnockoutPathPruneCombo(PDM pdm) {
        super(pdm, new EdgeWeight() {
            @Override
            public double get(Operation operation) {
                return operation.getCost() * operation.getTime();
            }
        }, new EdgeWeight() {
            @Override
            public double get(Operation operation) {
    ...
    @Override
     public Pair<Operation,Resource> execute(PDM pdm, LinkedHashSet<Operation> executable, LinkedHashSet<DataElement> available, HashMap<Operation, Boolean> prerun, Double current_time) {
         costGraph = pdm.graph(new EdgeWeight() {
            @Override
            public double get(Operation operation) {
                return operation.getLowestCombo(current_time);
            }});
        ...
        Resource resource = operation.getLowestComboResource(current_time);
